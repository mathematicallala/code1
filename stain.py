...
def achromatic_saturation_weighted(stain_achromatic_T, h, s, v, h0, w0):
    ...
    hist1 = hist1 / sum(sum(hist1))  # 消色区域的饱和权重色调直方图
    n1 = len(stain_h)
    ...
    hist2 = hist2 / sum(sum(hist2))#消色区域的饱和权重色调直方图
    n2 = len(achromatic_h)
    return hist1, n1, hist2, n2
def stain_achromatic_partition(stain_achromatic_T, h, s, I, h0, w0):#把色调图划分为染色区域和消色区域
    stain_h = []
    stain_s = []
    achromatic_h = []#表示消色区域
    achromatic_s = []#消色区域饱和度值b
    for i in range(h0):
        for j in range(w0):
...
def hue(h, s, h0, w0):#饱和权重色调直方图
    hist1 = np.zeros([1, 361], np.float32)
    for i in range(h0):
        for j in range(w0):
          thet = h[i, j]
          hist1[0, thet] = hist1[0, thet] + s[i, j]
    hist1 = hist1 / sum(sum(hist1))#染色区域的包和权重色调直方图
    return hist1
...
class GEM:
    ...
    def _phi(self, y, mu, sigma, skew):
        ...
        b = st.norm.cdf(skew * (y - mu) / math.sqrt(sigma), loc=0, scale=1)  # 标准正态分布的分布函数
        return 2 * a * b  # @是矩阵求内积
    ...
    def fit(self, data_s,data_a, hist, su, aa):
        ...
        step = 0
        L1 = 0
        while step < self.maxstep:#maxstep最大迭代次数

            step += 1
            # E步
            gamma_z = []
            gamma_t = []#存储隐变量期望值，即后验概率
            gamma_t2 = []
            L = 0
            for j in range(self.N):#N是数据总个数
                gamma_jz = []    # 依次求每个样本对K个分模型的响应度
                gamma_jt = []
                gamma_jt2 = []
            ...
            # k是聚类个数，j是像素个数
                ...
                # 更新权重
                self.alpha[k] = SUM / sum(sum(hist))  # 更新权重
...
#得到谱矩阵
def spectrum_matrix(I_i_bgr, c):#得到密度矩阵
...
def template(c):
    img00 = cv2.imread('F:\\datas\\result\\2_Template.png', 1)
...
if __name__ == '__main__':
    # start = time.time()
    file_name1 = 'F:\\datas\\result\\2_Source.png'
    img01 = cv2.imread(file_name1, 1)  # 读取的图像格式是bgr格式
